...
// 5-image editorial grid. Context labels for each shot.
const COMMUNITY_IMAGES = [
  {
    src: ASSETS.tennisWoman,
    alt: 'Woman on clay court with COCO',
    label: 'Clay court. 38°C. Still hydrated.',
    span: 'large',     // 7/12 cols top-left
  },
  {
    src: ASSETS.poolVilla,
    alt: 'Woman at Mykonos villa pool with COCO',
    label: 'Poolside, somewhere warm.',
    span: 'small-top', // 5/12 cols top-right
  },
  {
    src: ASSETS.padelMan,
    alt: 'Man after padel match drinking COCO',
    label: 'Post-match. Every time.',
    span: 'small-mid',
  },
  {
    src: ASSETS.telAvivBeach,
    alt: 'Woman at Tel Aviv beach with COCO',
    label: 'The promenade. Tel Aviv.',
    span: 'small-mid',
  },
  {
    src: ASSETS.pilatesWoman,
    alt: 'Woman at pilates studio with COCO',
    label: 'Before. During. After.',
    span: 'wide-bottom', // spans full bottom
  },
];
...
function CommunityCard({ src, alt, label, delay, className = '', style = {} }) {
  return (
    ...
    >
      ...
      <div className="absolute top-4 right-4 w-1.5 h-1.5 rounded-full bg-cream/60" />
    </motion.div>
  );
}
...
export default function CommunitySection() {
  ...
  return (
    ...
    >
      {/* Subtle background texture — very low opacity sage ring */}
      <div
      ...
      <div className="max-w-[1440px] mx-auto px-6 md:px-14">
        ...
        <div className="mb-16 md:mb-20 flex flex-col md:flex-row md:items-end md:justify-between gap-8">
          ...
          >
            From the court to the pool to the studio —
            the people who choose COCO choose it every day.
            Real moments. Real hydration.
          </motion.p>
        </div>

        {/* Desktop editorial grid — 12 cols, 2 rows */}
        <div className="hidden md:grid grid-cols-12 gap-4 md:gap-5" style={{ minHeight: '700px' }}>
          {/* Tennis woman — large, 7 cols, tall */}
          ...
          {/* Pool villa — top right, 5 cols */}
          <div className="col-span-5">
            <CommunityCard
              src={ASSETS.poolVilla}
              alt="Woman at Mykonos villa pool"
              label="Poolside, somewhere warm."
              delay={0.1}
              className="h-full"
          ...
          {/* Padel man + Tel Aviv — bottom right, split 2.5+2.5 */}
          <div className="col-span-5 grid grid-cols-2 gap-4 md:gap-5">
            <CommunityCard
              src={ASSETS.padelMan}
              alt="Man after padel with COCO"
              label="Post-match. Every time."
              delay={0.2}
              className="h-full"
        ...
        {/* Wide bottom image — pilates woman */}
        ...
        <div className="md:hidden flex flex-col gap-4">
          {[
            { src: ASSETS.tennisWoman,  alt: 'Tennis', label: 'Clay court. 38°C. Still hydrated.', ratio: '4/5' },
            { src: ASSETS.poolVilla,    alt: 'Pool',   label: 'Poolside, somewhere warm.',         ratio: '4/3' },
            { src: ASSETS.padelMan,     alt: 'Padel',  label: 'Post-match. Every time.',           ratio: '4/3' },
            { src: ASSETS.telAvivBeach, alt: 'Beach',  label: 'The promenade. Tel Aviv.',          ratio: '4/3' },
            { src: ASSETS.pilatesWoman, alt: 'Pilates','label': 'Before. During. After.',          ratio: '4/3' },
          ].map((img, i) => (
            <CommunityCard
              key={i}
              src={img.src}
        ...
        {/* Slim community tag row — no large gap before FinalCTA */}
        <motion.div
        ...
        >
          <p className="font-sans text-cream/30 text-xs tracking-[0.16em] uppercase">
            Share your moment · #cocowater · @drinkcoco
          </p>
          <a
            href="#order"
            className="font-sans text-cream/40 text-xs tracking-wide hover:text-cream/70 transition-colors duration-300"
          >
            Shop Now →
          </a>
        </motion.div>
